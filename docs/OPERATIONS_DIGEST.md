...
投影画面の月をクリックすると隠しメニューが開き、現場で投影を見ながら調整できます。短冊配置、短冊フォント、短冊フォントサイズ、入力カーソル、笹舟サイズ、笹舟下端オフセット、3D表示ON/OFF、短冊目標マーカーON/OFF、3Dボックス位置、笹左右XY、鑑賞地点、短冊目標、カメラ注視点、笹舟移動設定を1件だけ保存・呼び出しできます。この設定は投影端末ブラウザの `localStorage` に保存されます。フォントは端末内蔵フォントと、選択時に読み込む同梱縦書きフォントを併用します。

月メニューの「保存値をクリア」は、投影端末ブラウザに保存された月メニュー調整値と保存済み配置プリセットを削除し、管理画面/サーバー側の現在設定へ戻します。「全画面」はページ全体をブラウザの全画面表示へ切り替えます。解除は同じボタンまたはEscキーで行います。

管理画面の投影設定では、視差3D関連の項目だけが短いデバウンス後に即時プレビューされます。対象は視差ON/OFF、短冊目標マーカー、笹舟移動モード、左右移動倍率、前後/上下移動倍率、短冊奥行倍率、奥行基準順、3DボックスX/Y、鑑賞地点の左右/上下/距離、投影面余白、短冊目標X/Y、カメラ注視点X/Yです。短冊奥行は画面上Y位置ではなくz順だけから作られます。総スロット数、表示数、ローテーション間隔、風、雲、天の川演出などは、従来通り「適用」を押すまで投影画面へ反映されません。

管理画面は全端末で共有されるサーバ設定、月メニューは投影端末ごとの現場微調整です。管理画面だけで調整する主な項目は、表示数/入替数、タイプ速度、短冊ハイライト時間、風、天の川、雲、各シード、投影面余白、短冊奥行、奥行基準順です。月メニューだけで調整する主な項目は、短冊配置、文字サイズ、入力カーソル、笹左右XY、笹舟サイズ、笹舟下端余白、短冊中央整列、全画面です。3D ON/OFF、マーカー、3Dボックス、鑑賞地点、短冊目標、カメラ注視点、笹舟移動設定は両方から調整できます。

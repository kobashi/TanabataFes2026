...
投影画面の月をクリックすると隠しメニューが開き、現場で投影を見ながら調整できます。短冊配置、短冊フォント、短冊フォントサイズ、笹舟サイズ、笹舟下端オフセット、3D表示ON/OFF、消失点マーカーON/OFF、3Dボックス位置、笹左右XY、鑑賞地点、消失点、笹舟移動設定を1件だけ保存・呼び出しできます。この設定は投影端末ブラウザの `localStorage` に保存されます。フォントは端末内蔵フォントと、選択時に読み込む同梱縦書きフォントを併用します。

月メニューの「保存値をクリア」は、投影端末ブラウザに保存された月メニュー調整値と保存済み配置プリセットを削除し、管理画面/サーバー側の現在設定へ戻します。「全画面」は投影ステージをブラウザの全画面表示へ切り替えます。解除は同じボタンまたはEscキーで行います。

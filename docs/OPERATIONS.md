...
- 短冊のドラッグ配置と、投影画面の月メニューから保存する呼び出し用レイアウト1件は、投影端末ブラウザの `localStorage` に保存される。月メニューの保存/呼び出しには、短冊フォント、短冊フォントサイズ、笹舟サイズ、笹舟下端オフセットも含まれる。別端末や別ブラウザには自動共有されない。
...
消失点Yは投影面よりかなり下方まで設定できる。建物外壁などへ投影し、地表に鑑賞ポイントがある場合は、消失点Yを画面下端より下へ下げて調整する。

消失点だけではパース量は決まらない。鑑賞地点は左右、上下、壁面からの距離のXYZで調整する。鑑賞地点が壁面から近いほどパースは強く、遠いほど穏やかになる。鑑賞地点が正面から左右または上下にずれる場合は「鑑賞地点 左右」「鑑賞地点 上下」を調整する。
...
壁投影モードでは、鑑賞方向は消失点に固定される。消失点は投影の固定アンカーで、笹舟/カメラの左右・前後移動は通常通り短冊と笹の再投影差として反映される。奥行基準順は短冊Z間隔の基準であり、基準短冊の位置やスケールを固定する補正ではない。

消失点マーカーをONにすると、消失点マーカー、投影面余白のガイドライン、パース確認用のワイヤーフレーム3Dボックスが表示される。ガイドラインと3Dボックス表示はマーカー表示に連動する。
...
- 視差マーカーや余白ガイドが出ない場合: 管理画面の視差立体感と消失点マーカーがONか確認する。ガイドライン表示は消失点マーカーに連動する。

...
- 短冊のドラッグ配置と、投影画面の月メニューから保存する呼び出し用レイアウト1件は、投影端末ブラウザの `localStorage` に保存される。月メニューの保存/呼び出しには、短冊フォント、短冊フォントサイズ、笹舟サイズも含まれる。別端末や別ブラウザには自動共有されない。
...
消失点マーカーをONにすると、消失点マーカーと投影面余白のガイドラインが表示される。ガイドライン表示はマーカー表示に連動する。

総スロット数、表示数、移動量、文字アニメーション速度、ローテーション間隔、自動演出、天の川明るさ、雲の設定、雲シードは、入力中に「編集中」表示になり、「適用」ボタンで確定する。
